...
export default function Create({ brands, servers }: Props) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        manufacturer: '', // Ajout du champ fabricant
        power_rating: '', // Ajout de la puissance
    });
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            ...
            setSelectedImage(URL.createObjectURL(file)); // Affiche l'image sélectionnée
            setData('image', file);
        }
    };
    ...
    const handleServerSelection = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        setData('server_ids', selectedValues.map(Number)); // Convertir en nombre
    };
    ...
    return (
        ...
        >
            <h1>Ajouter une Solution de Refroidissement</h1>
            <form onSubmit={handleSubmit} encType="multipart/form-data">
                <div>
                    <label htmlFor="name">Nom de la Solution de Refroidissement:</label>
                    <input
                        type="text"
                        name="name"
                        value={data.name}
                ...
                <div className="mt-4">
                    ...
                    />
                    {errors.manufacturer && <div className="text-red-600 text-sm">{errors.manufacturer}</div>}
                </div>
                ...
                <div className="mt-4">
                    ...
                    <input
                        ...
                        className="mt-1 p-2 border rounded"
                    />
                    {selectedImage && <img src={selectedImage} alt="Prévisualisation" className="mt-2" width="150" />}
                ...
                <div className="mt-4">
                    ...
                    >
                        {processing ? 'Enregistrement...' : 'Ajouter la Solution de Refroidissement'}
                    </button>
                </div>
            </form>
        </AuthenticatedLayout>
