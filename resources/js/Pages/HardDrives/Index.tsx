...
export default function Index({ hardDrives }: Props) {
    ...
    const filteredHardDrives = hardDrives.filter((hardDrive) =>
        hardDrive.name.toLowerCase().includes(searchTerm)
    );

    return (
        <Layout>
            <h1 className="text-3xl font-semibold text-gray-800 mb-4 p-2">Liste des Disques Durs</h1>
                    ...
                    <tbody>
                        {filteredHardDrives.length > 0 ? (
                            filteredHardDrives.map((hardDrive) => (
                                <tr key={hardDrive.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                                    <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {hardDrive.name}
                                    </th>
                                    <td className="px-6 py-4">{hardDrive.brand.name}</td>
                                    ...
                                    <td className="px-6 py-4">{hardDrive.price ? `${hardDrive.price} €` : 'Non disponible'}</td>
                                    <td className="px-6 py-4">{hardDrive.type.toUpperCase()}</td>
                                    <td className="px-6 py-4">{hardDrive.capacity} Go</td>
                                    <td className="px-6 py-4">
                                        {hardDrive.image ? (
                                            <img
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex space-x-2">
                                            <Link href={`/hard-drives/${hardDrive.id}/edit`} className="font-medium text-green-600 hover:underline">
                                                Modifier
                                            </Link>
                                            <button
                                            ...
                                            <Link href={`/hard-drives/${hardDrive.id}`} className="font-medium text-blue-600 hover:underline">
                                                Voir
                                            </Link>
                                        </div>
                                    </td>
                            ...
                            <tr>
                                <td colSpan={8} className="px-6 py-4 text-center text-gray-500">Aucun disque dur trouvé</td>
                            </tr>
                        )}
                    </tbody>
                </table>
