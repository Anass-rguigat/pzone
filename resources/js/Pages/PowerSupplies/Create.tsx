...
export default function Create({ brands, servers }: Props) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        efficiency: '', // Ajout du champ efficacité
        form_factor: '', // Ajout du champ form factor
        modular: false, // Ajout du champ modular
        price: '',
    });
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            ...
            setSelectedImage(URL.createObjectURL(file)); // Affichage de l'image sélectionnée
            setData('image', file);
        }
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post('/power-supplies', {
            onSuccess: () => {
                // Réinitialisation du formulaire et de l'image sélectionnée
                setData({
                    name: '',
                    brand_id: '',
                    image: null,
    ...
    return (
        ...
        >
            <h1>Ajouter une alimentation (Power Supply)</h1>
            <form onSubmit={handleSubmit} encType="multipart/form-data">
                <div>
                    <label htmlFor="name">Nom de l'alimentation:</label>
                    <input
                        type="text"
                        name="name"
                        value={data.name}
                ...
                <div className="mt-4">
                    <label htmlFor="capacity">Capacité (W):</label>
                    <input
                        type="number"
                        name="capacity"
                        value={data.capacity}
                ...
                <div className="mt-4">
                    ...
                    />
                    {errors.efficiency && <div className="text-red-600 text-sm">{errors.efficiency}</div>}
                </div>
                ...
                <div className="mt-4">
                    <label htmlFor="modular">Modulaire:</label>
                    <input
                        type="checkbox"
                        name="modular"
                        ...
                        className="mt-1 p-2 border rounded"
                    />
                    {errors.modular && <div className="text-red-600 text-sm">{errors.modular}</div>}
                </div>
                    ...
                    <input
                        ...
                        onChange={handleChange}
                        className="mt-1 p-2 border rounded"
                        placeholder="Prix de l'alimentation"
                    />
                ...
                <div className="mt-4">
                    ...
                    >
                        {processing ? 'Enregistrement...' : 'Ajouter l\'alimentation'}
                    </button>
                </div>
            </form>
        </AuthenticatedLayout>
