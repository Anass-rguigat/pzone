...
export default function Index({ batteries }: Props) {
    ...
    return (
        <Layout>
            <h1 className="text-3xl font-semibold text-gray-800 mb-4 p-2">Liste des Batteries</h1>
                    ...
                    <tbody>
                        {filteredBatteries.length > 0 ? (
                            filteredBatteries.map((battery) => (
                                <tr key={battery.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                                    <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {battery.name}
                                    </th>
                                    <td className="px-6 py-4">{battery.brand.name}</td>
                                    ...
                                    <td className="px-6 py-4">{battery.price} &euro;</td>
                                    <td className="px-6 py-4">{battery.type}</td>
                                    <td className="px-6 py-4">{battery.capacity} Wh</td>
                                    <td className="px-6 py-4">
                                        {battery.image ? (
                                            <img 
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex space-x-2">
                                            <Link href={`/batteries/${battery.id}/edit`} className="font-medium text-green-600 dark:green-blue-500 hover:underline">
                                                Modifier
                                            </Link>
                                            <button
                                            ...
                                            <Link href={`/batteries/${battery.id}`} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                                Voir
                                            </Link>
                                        </div>
                                    </td>
                            ...
                            <tr>
                                <td colSpan={8} className="px-6 py-4 text-center text-gray-500">Aucune batterie trouvée</td>
                            </tr>
                        )}
                    </tbody>
                </table>
